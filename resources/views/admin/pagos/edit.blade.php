...
@section('content')
    ...
    <h2 style="margin-bottom: 30px;">Editar Pago #{{ $pago->id_pago }}</h2>
    ...
    <form action="{{ route('admin.pagos.update', $pago->id_pago) }}" method="POST" style="max-width: 800px;">
        @csrf
        @method('PUT')
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                ...
                <select name="id_reserva" required
                    ...
                    @foreach($reservas as $reserva)
                        <option value="{{ $reserva->id_reserva }}" 
                            {{ old('id_reserva', $pago->id_reserva) == $reserva->id_reserva ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="number" name="monto" value="{{ old('monto', $pago->monto) }}" required min="0" step="0.01"
                       style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 5px;">
            </div>
            ...
            <div>
                ...
                <select name="metodo" required
                    ...
                    <option value="yape" {{ old('metodo', $pago->metodo) == 'yape' ? 'selected' : '' }}>Yape</option>
                    <option value="plin" {{ old('metodo', $pago->metodo) == 'plin' ? 'selected' : '' }}>Plin</option>
                    <option value="paypal" {{ old('metodo', $pago->metodo) == 'paypal' ? 'selected' : '' }}>PayPal</option>
            ...
            <div>
                ...
                <select name="estado_pago" required
                    ...
                    <option value="completado" {{ old('estado_pago', $pago->estado_pago) == 'completado' ? 'selected' : '' }}>Completado</option>
                    <option value="fallido" {{ old('estado_pago', $pago->estado_pago) == 'fallido' ? 'selected' : '' }}>Fallido</option>
                    <option value="reembolsado" {{ old('estado_pago', $pago->estado_pago) == 'reembolsado' ? 'selected' : '' }}>Reembolsado</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" name="referencia_pago" value="{{ old('referencia_pago', $pago->referencia_pago) }}" maxlength="100"
                       style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 5px;">
            </div>
        </div>
