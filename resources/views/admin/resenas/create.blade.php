...
@section('page-title', 'Crear Nueva Reseña')
...
@section('content')
    ...
    <h2 style="margin-bottom: 30px;">Crear Nueva Reseña</h2>
    ...
    <form action="{{ route('admin.resenas.store') }}" method="POST" style="max-width: 800px;">
        @csrf

        <div style="margin-bottom: 20px;">
            <!-- Reserva -->
            <div style="margin-bottom: 20px;">
                <label style="display: block; margin-bottom: 5px; font-weight: 600;">Reserva Completada *</label>
                <select name="id_reserva" required
                    ...
                    @forelse($reservas as $reserva)
                        <option value="{{ $reserva->id_reserva }}" {{ old('id_reserva') == $reserva->id_reserva ? 'selected' : '' }}>
                            #{{ $reserva->id_reserva }} - {{ $reserva->hospedaje->titulo }} - {{ $reserva->usuario->nombre }} {{ $reserva->usuario->apellido }}
                        </option>
                    @empty
                        <option value="">No hay reservas completadas sin reseña</option>
                    @endforelse
                </select>
                <small style="color: #666;">Solo se muestran reservas completadas sin reseña</small>
            </div>
                    ...
                    @for($i = 1; $i <= 5; $i++)
                        <label style="display: flex; align-items: center; gap: 5px; cursor: pointer;">
                            <input type="radio" name="calificacion" value="{{ $i }}" 
                                {{ old('calificacion') == $i ? 'checked' : '' }} required>
            ...
            <div>
                ...
                <textarea name="comentario" rows="6"
                          style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 5px;"
                          placeholder="Escribe tu experiencia en el hospedaje...">{{ old('comentario') }}</textarea>
            </div>
        </div>
